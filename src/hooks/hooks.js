  ...
  return {
    ...
    alter: {
      ...
      emailTransports: function(transports, settings) {
        var office365 = _prop('office365')(settings) || {};
      ...
      isOwner: function(_isOwner, ownerId, req) {
        return req.projectOwner && (ownerId === req.projectOwner);
      },
      defaultRoles: function(roles, access) {
        return [access.project.default];
      },
      ...
      hasAccess: function(_hasAccess, req, access) {
        if (req.token && access.hasOwnProperty('project') && access.project.hasOwnProperty('owner')) {
          if (req.token.user._id === access.project.owner) {
            if ((req.method === 'POST' || req.method === 'PUT') && req.body.hasOwnProperty('owner') && req.body.owner) {
      ...
      requestParams: function(req, params) {
        var projectId = formioServer.formio.util.getUrlParams(req.baseUrl).project;
      ...
      formQuery: function(query, req) {
        // Do not allow any forms to show without projects.
        req.projectId = req.projectId || req.params.projectId || 0;

        // Add the projects to the form queries.
